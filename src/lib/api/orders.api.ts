...
import type { CreateOrderPayload, Order, PaginationMeta, TrackedOrder } from "@/types";

export const createOrder = (body: CreateOrderPayload) =>
  apiPost<{ order: Order; nextStep: string | null }>("/orders", body);

export const listOrders = async (params: { page?: number; limit?: number; status?: string } = {}) => {
